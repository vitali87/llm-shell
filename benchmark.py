...
class OllamaBatchProcessor:
    ...
    def process_single_prompt(self, prompt: str, attempt: int = 0) -> dict[str, Any]:
        ...
        enhanced_prompt = (
            "Translate this command to its actual shell command(s). "
            f"Return ONLY the command, no explanations: {prompt}"
        )
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/chat", headers=headers, json=data, timeout=30
            )
    ...
    def process_batch(self, prompts: list[str]) -> list[dict[str, Any]]:
        ...
        """
        logger.info(
            f"Processing {len(prompts)} prompts with {self.max_workers} workers"
        )
    ...
    if path.suffix.lower() == ".json":
        with open(path, encoding="utf-8") as f:
            ...
            if isinstance(data, list):
                ...
                raise ValueError(
                    "JSON file must contain a list of prompts "
                    "or a dict with 'prompts' key"
                )
    else:
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", default="output.json", help="Output file path"
    )
    parser.add_argument("--model", "-m", default="mistral", help="Ollama model name")
    parser.add_argument(
        "--workers", "-w", type=int, default=10, help="Number of worker threads"
    )
    parser.add_argument(
        "--temperature",
        "-t",
    ...
    parser.add_argument(
        "--base-url", "-u", default="http://localhost:11434", help="Ollama API base URL"
    )
    ...
    try:
        ...
        logger.info(
            f"Processing complete. Generated commands for {len(results)} instructions."
        )
